...
class InfoNaruDbInsertTest {
    ...
    void api_to_db_insert() {
        ...
        params.put("sort", "loan");
        params.put("format", "json");

        List<BookIsbnTempDTO> books = infoNaruClient.getBookList(params);
        ...
        BookIsbnTempDTO first = books.get(0);
        System.out.println("----- first book from API -----");
        System.out.println("isbn13   = " + first.getIsbn13());
        System.out.println("booktile = " + first.getBookTitle());
        System.out.println("publisher= " + first.getPublisher());
        System.out.println("image  = " + first.getImage());
        System.out.println("infonarufetchedat  = " + first.getInfonaruFetchedAt());
        System.out.println("--------------------------------");
        ...
        // 4) DB insert
        int inserted = bookMapper.insertBookIsbnTemp(books);
        ...
        Integer exists = jdbcTemplate.queryForObject(
                "select count(*) from book_isbn_temp where isbn13 = ?",
                Integer.class,
                first.getIsbn13()
        );
